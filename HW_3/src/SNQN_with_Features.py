...
def parse_args():
    ...
    parser.add_argument(
        "--r_click", type=float, default=0.2, help="reward for the click behavior."
    )
    parser.add_argument(
        "--r_buy", type=float, default=1.0, help="reward for the purchase behavior."
    )
    parser.add_argument(
        "--r_negative",
        type=float,
    ...
    parser.add_argument(
        "--discount", type=float, default=0.5, help="Discount factor for RL."
    )
    parser.add_argument(
        "--neg", type=int, default=10, help="number of negative samples."
    )
    parser.add_argument(
        "--weight", type=float, default=1.0, help="number of negative samples."
    )
    parser.add_argument(
        "--model",
        type=str,
    ...
    parser.add_argument(
        "--num_heads", default=1, type=int, help="number heads (for SASRec)"
    )
    parser.add_argument(
        "--num_blocks", default=1, type=int, help="number heads (for SASRec)"
    )
    parser.add_argument("--dropout_rate", default=0.1, type=float)
    parser.add_argument("--mixing_parameter", default=0.2, type=float)  ####
...
class QNetwork:
    def __init__(
        ...
        with tf.variable_scope(self.name):
            ...
            self.inputs = tf.placeholder(
                tf.int32, [None, state_size]
            )  # sequence of history, [batchsize,state_size]
            self.len_state = tf.placeholder(
                tf.int32, [None]
            ...
            if self.model == "Caser":
                mask = tf.expand_dims(
                    tf.to_float(tf.not_equal(self.inputs, item_num)), -1
                )
                ...
                self.h_pool_flat = tf.reshape(
                    self.h_pool, [-1, num_filters_total]
                ...
                self.final = tf.concat(
                    [self.h_pool_flat, self.vcnn_flat], 1
                )  # shape=[batch_size, 384+100]

                # Add dropout
                with tf.name_scope("dropout"):
                    self.states_hidden = tf.layers.dropout(
                        self.final,
            ...
            if self.model == "NItNet":
                ...
                for layer_id, dilation in enumerate(self.model_para["dilations"]):
                    dilate_output = nextitnet_residual_block(
                        dilate_output,
                        dilation,
                        layer_id,
                        self.model_para["dilated_channels"],
            ...
            if self.model == "SASRec":
                pos_emb = tf.nn.embedding_lookup(
                    self.all_embeddings["pos_embeddings"],
                    tf.tile(
                        tf.expand_dims(tf.range(tf.shape(self.inputs)[1]), 0),
            ...
            self.output2 = tf.compat.v1.layers.dense(
                self.states_hidden, self.item_num, activation=None, use_bias=True
            )  # all ce logits     ####

            self.w_fj = tf.compat.v1.layers.dense(
                self.item_features, self.hidden_size, activation=None, use_bias=True
            )  ####

            self.phi_hk_fj = tf.matmul(
                self.states_hidden, self.w_fj, transpose_b=True
            )  ####

            self.score = (
                1 - self.mixing_parameter
            ) * self.output2 + self.mixing_parameter * self.phi_hk_fj  ####
            ...
            self.targetQs_selector = tf.placeholder(
                tf.float32, [None, item_num]
            ...
            self.targetQ_current_selector = tf.placeholder(
                tf.float32, [None, item_num]
            )  # used for select best action for double q learning

            # TRFL double qlearning
            qloss_positive, _ = trfl.double_qlearning(
                self.output1,
                self.actions,
                self.reward,
                self.discount,
                self.targetQs_,
                self.targetQs_selector,
            )
            neg_reward = tf.constant(
                reward_negative, dtype=tf.float32, shape=(args.batch_size,)
            )
            qloss_negative = 0
            for i in range(self.neg):
                negative = tf.gather(self.negative_actions, i, axis=1)

                qloss_negative += trfl.double_qlearning(
                    self.output1,
                    negative,
                    neg_reward,
                    self.discount,
                    self.targetQ_current_,
                    self.targetQ_current_selector,
                )[0]

            ce_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(
                labels=self.actions, logits=self.score
            )  #####
    ...
    def initialize_embeddings(self):
        ...
        if self.pretrain == False:
            with tf.variable_scope(self.name):
                state_embeddings = tf.Variable(
                    tf.random_normal([self.item_num + 1, self.hidden_size], 0.0, 0.01),
                    name="state_embeddings",
                )
                pos_embeddings = tf.Variable(
                    tf.random_normal([self.state_size, self.hidden_size], 0.0, 0.01),
                    name="pos_embeddings",
                )
...
if __name__ == "__main__":
    ...
    )  # read data statistics, includeing state_size and item_num
    item_features = pd.read_csv(
        os.path.join(data_directory, "item_features.csv")
    ).to_numpy(dtype=np.float32)
    lambda_ = args.mixing_parameter  ####
    state_size = data_statis["state_size"][
        0
    ]  # the length of history to define the state
    ...
    with tf.Session() as sess:
        ...
        for i in range(args.epoch):
            for j in range(num_batches):
                ...
                target_Q_current = sess.run(
                    target_QN.output1,
                    feed_dict={
                        target_QN.inputs: state,
                        target_QN.len_state: len_state,
                        target_QN.is_training: True,
                    },
                )
                target_Q__current_selector = sess.run(
                    mainQN.output1,
                    feed_dict={
                        mainQN.inputs: state,
